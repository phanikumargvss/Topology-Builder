...
def get_ndp_output(current_node, usr, pwd, file_append, output_nodes, output_links):
    ...
    if (nos == 'ios'):
        ...
        if 'SSH_Error' in cdp_out:
            ...
            unique = ios_cdp.parser(
                cdp_out, current_node,  file_append, output_links)

            lldp_out = access.ssh_ndp(
                current_node, 'show lldp neighbor', usr, pwd)
            if "enabled" not in lldp_out:
                lldp = ios_lldp.parser(
                    lldp_out, current_node, unique,  file_append, output_links)
                unique.extend(lldp)
    elif (nos == 'nxos'):
        cdp_out = access.ssh_ndp(current_node, 'show cdp neighbor', usr, pwd)
        if 'SSH_Error' in cdp_out:
            ...
            unique = nxos_cdp.parser(
                cdp_out, current_node,  file_append, output_links)

            lldp_out = access.ssh_ndp(
                current_node, 'show lldp neighbor', usr, pwd)
            lldp = nxos_lldp.parser(
                lldp_out, current_node, unique,  file_append, output_links)
            unique.extend(lldp)
    elif (nos == 'iosxr'):
        cdp_out = access.ssh_ndp(current_node, 'show cdp neighbor', usr, pwd)
        if 'SSH_Error' in cdp_out:
            ...
            unique = iosxr_cdp.parser(
                cdp_out, current_node,  file_append, output_links)

            lldp_out = access.ssh_ndp(
                current_node, 'show lldp neighbor', usr, pwd)
            lldp = iosxr_lldp.parser(
                lldp_out, current_node, unique,  file_append, output_links)
            unique.extend(lldp)
    elif (nos == 'catos'):
        cdp_out = access.ssh_ndp(current_node, 'show cdp neighbor', usr, pwd)
        if 'SSH_Error' in cdp_out:
            ...
            unique = catos.parser(cdp_out, current_node,
                                  file_append, output_links)
    elif (nos == 'eos'):
        lldp_out = access.ssh_ndp(current_node, 'show lldp neighbor', usr, pwd)
        if 'SSH_Error' in lldp_out:
            error_code = lldp_out
        else:
            unique = eos.parser(lldp_out, current_node,
                                file_append, output_links)
    elif (nos == 'junos'):
        lldp_out = access.ssh_ndp(current_node, 'show lldp neighbor', usr, pwd)
        if 'SSH_Error' in lldp_out:
            ...
            unique = junos.parser(lldp_out, current_node,
                                  file_append, output_links)

    access_time = time.time() - access_start_time
    accessed_file.write(current_node + "\t" + nos + "\t" +
                        str("%0.2f" % access_time) + "\t" + error_code + "\n")
