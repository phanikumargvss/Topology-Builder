...
#	print "\n\tnext_nodes from multi_access:", next_nodes, "(%d)" %count
    ...
    for i in range(len(next_nodes)):
        node_device_type = next_nodes[i][next_nodes[
            i].find("-") + 1: next_nodes[i].find("-") + 3]
...
#		print "\n\tnext_nodes from multi_access:", next_nodes[i]
    ...
    for i in range(len(filtered_next_nodes)):
        process = mp.Process(target=discover.get_ndp_output, args=(filtered_next_nodes[
                             i], username, password, file_append, output_nodes, output_links))
        process.daemon = True
...
def main():
    ...
    #----------------------------------------------------------------
    process = mp.Process(target=links_writer, args=(
        output_links,))				# Spawn 'links_writer' process
    # process.daemon = True
    # # If this process is daemon, writing to file doesn't work
    process.start()

    print "\n"
    #----------------------------------------------------------------
    # Start crawl from seed device
    next_access([seed_device], output_nodes, output_links)
...
#	print "\nTotal # of processes:", len(all_processes)
    ...
    # End crawl: signal to 'links_writer' to release file
    output_links.put('end_crawl')
    #----------------------------------------------------------------
    ...
    nodes_dict = node_sort.node_sort(
        upper_limit, lower_limit, location_code, file_append)
    ...
    plot_small.plot(upper_limit, lower_limit, location_code,
                    file_append, interface_labels, graph_type)
